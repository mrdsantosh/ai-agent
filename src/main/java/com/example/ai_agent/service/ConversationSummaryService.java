...
public class ConversationSummaryService {
	...
	public ConversationSummaryService(ChatMemoryService chatMemoryService, ChatClient.Builder chatClientBuilder) {
	...
	public String summarizeAndSave(String conversationId) {
		...
		String messagesText = messages.stream().filter(msg -> msg.getText() != null && !msg.getText().isEmpty())
				.map(msg -> msg.getMessageType() + ": " + msg.getText()).reduce((a, b) -> a + "\n" + b).orElse("");
		...
		if (!preferences.isEmpty()) {
			chatMemoryService.getPreferencesMemory().add(conversationId + "_preferences",
					new AssistantMessage(preferences));
		}

		if (!context.isEmpty()) {
			chatMemoryService.getContextMemory().add(conversationId + "_context",
					new AssistantMessage("[" + timestamp + "] " + context));
		}
	...
	private String extractSection(String response, String startMarker, String endMarker) {
		try {
			...
			int end = endMarker != null && response.contains(endMarker) ? response.indexOf(endMarker)
					: response.length();
