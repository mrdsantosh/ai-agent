...
public class ChatMemoryService {
	...
	// Three-tier memory: Session (recent), Context (summaries), Preferences
	// (profile)
	...
	public ChatMemoryService(DataSource dataSource) {
		...
		var jdbcRepository = JdbcChatMemoryRepository.builder().dataSource(dataSource)
				.dialect(new PostgresChatMemoryRepositoryDialect()).build();

		this.sessionMemory = MessageWindowChatMemory.builder().chatMemoryRepository(jdbcRepository)
				.maxMessages(MAX_SESSION_MESSAGES).build();

		this.contextMemory = MessageWindowChatMemory.builder().chatMemoryRepository(jdbcRepository)
				.maxMessages(MAX_CONTEXT_SUMMARIES).build();

		this.preferencesMemory = MessageWindowChatMemory.builder().chatMemoryRepository(jdbcRepository)
				.maxMessages(MAX_PREFERENCES).build();
	}
	...
	private void loadPreviousContext(String conversationId, ChatClient chatClient) {
		...
		var chatResponse = chatClient.prompt().user("Summarize this user information concisely:\n\n" + contextBuilder)
				.call().chatResponse();

		String contextSummary = (chatResponse != null && chatResponse.getResult() != null
				&& chatResponse.getResult().getOutput() != null) ? chatResponse.getResult().getOutput().getText()
						: null;
		...
		String contextMessage = String
				.format("You are continuing a conversation with this user. Here is what you know:\n\n%s\n\n"
						+ "Use this information to provide personalized responses.", contextSummary);
