...
public class WeatherService {
	...
	public String getWeather(String city, String date) {
		...
		try {
			...
			Map<?, ?> geocodingResponse = webClient.get().uri(geocodingUrl).retrieve().bodyToMono(Map.class)
			...
			logger.info("Retrieved weather for {}: min: {}{}, max: {}{}", locationDisplay, minTemp, formattedUnit,
					maxTemp, formattedUnit);

			return String.format("Weather for %s on %s:\nMin: %.1f%s, Max: %.1f%s", locationDisplay, date, minTemp,
